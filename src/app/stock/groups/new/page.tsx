...
export default function StockGroupNewPage() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    // Şimdilik sadece UI — daha sonra veritabanına kaydedeceğiz
  ...
  return (
    <main style={{ minHeight: '100dvh', background: 'linear-gradient(135deg,#0b2161,#0e3aa3)', color: 'white' }}>
      <section style={{ padding: 16 }}>
        <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 8 }}>Ana Sayfa / Stok Kartları / Gruplar / Yeni</div>
        ...
        <div style={{ padding: 16, borderRadius: 16, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', backdropFilter: 'blur(10px)', maxWidth: 520 }}>
          <div style={{ fontSize: 18, marginBottom: 12 }}>Yeni Stok Grubu Oluştur</div>

          <form onSubmit={submit} style={{ display: 'flex', gap: 8 }}>
            <input
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="Grup adınızı..."
